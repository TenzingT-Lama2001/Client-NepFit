...
interface FormValuesProps extends Omit<Trainer, "avatarUrl"> {
  avatarUrl: CustomFile | string | null;
}

export default function TrainerNewEditForm({
  isEdit = false,
  currentTrainer,
}: Props) {
  ...
  let NewTrainerSchema;
  if (isEdit) {
    NewTrainerSchema = Yup.object().shape({
      ...
      role: Yup.string().oneOf(["trainer"], 'Role must be "trainer"'),
    ...
    NewTrainerSchema = Yup.object().shape({
      ...
      role: Yup.string()
        ...
        .oneOf(["trainer"], 'Role must be "trainer"'),
      avatarUrl: Yup.string(),
      specialty: Yup.string().required("Please enter trainer's specialty"),
      classTime: Yup.string(),
    });
  }
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      avatarUrl: currentTrainer?.avatarUrl?.secure_url || "",
      password: currentTrainer?.password || "",
      specialty: currentTrainer?.specialty || "",
      classTime: currentTrainer?.classTime || "",
    }),
    [currentTrainer]
  );
  ...
  const onSubmit = async (data: FormValuesProps) => {
    try {
      ...
      isEdit
        ? updateTrainerMutation.mutate(newEditData)
        : createTrainerMutation.mutate(newData);
    } catch (err) {
      console.log(err);
    }
  };
        ...
        <Grid item xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFTextField name="specialty" label="Specialty" />
              <RHFTextField name="classTime" label="Class Time" />
            </Box>
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              ...
              >
                {!isEdit ? "Create Trainer" : "Save Changes"}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
