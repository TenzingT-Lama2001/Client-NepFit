...
import { getApprovedBooking } from "../../../../api/booking";
import RequestListTableRow from "../../../../sections/auth/dashboard/booking/RequestListTableRow";
...
RequestList.getLayout = function getLayout(page: React.ReactElement) {
...
export default function RequestList() {
  ...
  const { data, isLoading, refetch } = useQuery<any>(
    ["get_approved_booking", trainerId],
    () => getApprovedBooking(trainerId),
    {
      onSuccess(data) {
        console.log("GET APPROVED BOOKING", data);
        setTableData(data);
      },
    }
  );
      ...
      <Container maxWidth="lg">
        <HeaderBreadcrumbs
          ...
          links={[
            ...
            { name: "Trainer", href: PATH_DASHBOARD.dashboard.staff.root },
            { name: "Request" },
          ]}
        />
              ...
              <TableBody>
                {dataFiltered.length > 0 ? (
                  dataFiltered.map((row: any) => (
                    <RequestListTableRow key={row._id} row={row} />
                  ))
                ) : (
